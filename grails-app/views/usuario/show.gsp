	...
	<body>
		...
		<div id="show-usuario" class="content scaffold-show" role="main">
			...
			<ol class="property-list usuario">
				...
				<g:if test="${usuarioInstance?.hashSenha}">
				<li class="fieldcontain">
					<span id="hashSenha-label" class="property-label"><g:message code="usuario.hashSenha.label" default="Hash Senha" /></span>
					
						<span class="property-value" aria-labelledby="hashSenha-label"><g:fieldValue bean="${usuarioInstance}" field="hashSenha"/></span>
					
				</li>
				</g:if>
			
				<g:if test="${usuarioInstance?.links}">
				<li class="fieldcontain">
					<span id="links-label" class="property-label"><g:message code="usuario.links.label" default="Links" /></span>
					
						<g:each in="${usuarioInstance.links}" var="l">
						<span class="property-value" aria-labelledby="links-label"><g:link controller="link" action="show" id="${l.id}">${l?.encodeAsHTML()}</g:link></span>
						</g:each>
					
				</li>
				</g:if>
				
				<g:if test="${usuarioInstance?.locais}">
				<li class="fieldcontain">
					<span id="locais-label" class="property-label"><g:message code="usuario.locais.label" default="Locais" /></span>
					
						<g:each in="${usuarioInstance.locais}" var="c">
						<span class="property-value" aria-labelledby="locais-label"><g:link controller="coordenada" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
